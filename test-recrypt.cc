...
main(int argc, char *argv[])
{
  ...
  //logDim = 11 for the small setting
  //logDim = 13 for the medium setting
  //logDim = 15 for the large setting
  long logDim = 13;
  long nBits =384;
  int ops=2, mode=0;
  ...
  keys.gpuEncrypt(c,1);
  timer += GetTime();
  cout<<"enc: "<<timer<<"s"<<endl;
  cerr << "c = enc(1) decrypted as " << keys.decrypt(c) <<endl;
  timer = - GetTime();
  keys.gpuEncrypt(c,1);
  timer += GetTime();
  cout<<"enc: "<<timer<<"s"<<endl;
  ...
  timer += GetTime();
  cout<<"enc: "<<timer<<"s"<<endl;
  cerr << "c =decrypted as " << keys.decrypt(c) <<endl;
  keys.ReleasePreEnc();
	
  keys.gpuPreComputation2();
  ...
  for (int i=0; i<8; i++) {
    ...
    keys.gpuRecrypt(c);
    timer += GetTime();
    cout<<"recrypt: "<<timer<<"s"<<endl;
    cerr << "c = reCrypt(c^2) decrypted as " << keys.decrypt(c) <<endl;
    cerr << "c bits = " << NumBits(c)<<endl;
  }
}
